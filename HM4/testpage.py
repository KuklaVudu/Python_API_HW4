...
class OperationHelper(BasePage):
    def enter_text_into_field(self, locator, word, description=None):
        ...
        self.logger.exception(f'Отправить {word} в элемент {element_name}')
        field = self.find_element(locator)
        if not field:
            self.logger.exception(f'Элемент {locator} не найден')
            return False
        try:
            field.clear()
            field.send_keys(word)
        except:
            self.logger.exception(f'Исключение при работе с {locator}')
            return False
        return True
    ...
    def click_button(self, locator, description=None):
        ...
        try:
            ...
            self.logger.exception(f'Исключение при нажатии')
            return False
        self.logger.debug(f'Нажал {element_name} кнопку')
        return True
    ...
    def get_text_from_element(self, locator, description=None):
        ...
        try:
            ...
            self.logger.exception(f'Исключение при получении теста от {element_name}')
            return None
        self.logger.debug(f'Находим текст {text} в поле {element_name}')
        text = field.text
    ...
    def enter_content(self, word):
        self.enter_text_into_field(TestSearchLocators.dictloc["LOCATOR_CONTENT_FIELD"], word,
                                   description="post content form")

    def enter_name(self, word):
        self.enter_text_into_field(TestSearchLocators.dictloc["LOCATOR_CONTACT_NAME_FIELD"], word,
    ...
    def get_profile_text(self):
        return self.get_text_from_element(TestSearchLocators.dictloc["LOCATOR_USER_PROFILE_NAME"],
                                          description="user profile name")

    def get_alert_text(self):
        self.logger.exception.info("Получить текст оповещения")
        text = self.alert()
        self.logger.exception.info(text)
        return text